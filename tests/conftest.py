...
def patch_db(in_memory_conn, monkeypatch):
    """
    Redirect every get_connection() call in database to the shared in-memory
    connection so no file I/O happens during tests.
    """
    import database
...
@pytest.fixture
def sample_price_df():
    """Minimal price DataFrame using FinMind column names, matching what load_stock_data returns."""
    dates = pd.date_range("2024-01-02", periods=5, freq="B")
    return pd.DataFrame(
        {
            "open":           [100.0, 101.0, 102.0, 101.5, 103.0],
            "max":            [101.0, 102.0, 103.0, 102.5, 104.0],
            "min":            [ 99.0, 100.0, 101.0, 100.5, 102.0],
            "close":          [100.5, 101.5, 102.5, 101.0, 103.5],
            "Trading_Volume": [1000.0, 1200.0, 900.0, 1100.0, 1300.0],
        },
