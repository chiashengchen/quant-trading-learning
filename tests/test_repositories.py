"""
Tests for repositories.py — save, load, and latest-date queries.
All DB calls go to an in-memory SQLite via the patch_db fixture.
"""
import pandas as pd
import pytest

from repositories import save_daily_prices, load_daily_prices, get_latest_price_date
...
class TestSaveDailyPrices:
    ...
    def test_upsert_does_not_duplicate(self, patch_db, sample_price_df):
        save_daily_prices("0050", sample_price_df)
        save_daily_prices("0050", sample_price_df)  # second insert should replace
...
class TestGetLatestPriceDate:
    ...
    def test_returns_max_date(self, patch_db, sample_price_df):
        ...
        assert latest == "2024-01-08"  # last business day in the 5-day range
